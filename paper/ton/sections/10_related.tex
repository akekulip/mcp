...
RFC 6374~\cite{rfc6374} & TX/RX counter pairs & in-fabric, passive & if measured per DL & attribution fails & epoch-skew limited (measured here) & none under spray & no \\
Synchronized snapshot (RFC 6374 with PTP-latched shadow banks) & counter pairs latched on a common epoch & in-fabric, passive & exact DL if per-DL & attribution needs the spray field & matches the witness if latch skew is below about 10 µs; costs a second register bank, a bank swap, and fabric-wide time (Section~\ref{sec:discussion}) & exact DL & no \\
