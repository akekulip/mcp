...
Detection answers whether something is wrong; localization answers which directed link, which is the question a repair needs. We expect the ledger to name exactly one directed link at every rate, because it holds each link's own count, and we expect the passive arms to degrade to the uplink and downlink pair once corroborating traffic runs short (Section~\ref{sec:limits}). Table~\ref{tab:localization} presents the outcome for a downlink fault and for an uplink fault, over 50 seeds per cell on the two-hop fabric of Section~\ref{sec:method}, and Figure~\ref{fig:localization} plots the exact-localization rates and the size of the named set.
