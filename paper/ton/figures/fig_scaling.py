...
for arm in ARMS:
    ...
    for i, pk, a in pts:
        if pk["median"] is None:
            ...
            if not full:
                ax.annotate(f"{a:.2f}", (i, pk["median"]), textcoords="offset points", xytext=(6, -3), fontsize=ANNOT, color=COLORS[arm])
ax.axhline(BUDGET, color="0.4", linestyle=":", linewidth=1.0, zorder=1)
ax.text(len(RATES8) - 1.0, BUDGET * 1.25, "budget: 80 epochs, 160 M", fontsize=ANNOT, color="0.35", ha="right")
ax.axvline(len(RATES8) - 1, color="0.75", linewidth=0.8, zorder=0)
ax.text(len(RATES8) - 1.08, 2.3e6, "floor $f=10^{-5}$", fontsize=ANNOT, color="0.35", ha="right", rotation=90, va="bottom")
ax.set_yscale("log")
ax.set_xticks(x); ax.set_xticklabels([fmt_p(p) if p >= 0.005 else f"$10^{{{np.log10(p):.0f}}}$" if np.log10(p) == int(np.log10(p)) else f"${p*1e5:g}\\times10^{{-5}}$" for p in RATES8], rotation=30, ha="right", fontsize=ANNOT)
...
ax.legend(loc="upper left", ncol=2, framealpha=1.0, fontsize=ANNOT, columnspacing=0.9, handlelength=1.6)
