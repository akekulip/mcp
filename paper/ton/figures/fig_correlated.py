...
fig, axes2 = plt.subplots(2, 2, figsize=(7.16, 3.7))
axes = axes2.ravel()
w = 0.26
        ...
        for xi, v in zip(x + (i - 1) * w, y):
            if v > 0:
                ax.text(xi, v + (0.02 if field != "final" else 0.6), f"{v:.2f}" if v < 10 else f"{v:.0f}",
                        ha="center", va="bottom", fontsize=ANNOT, color="0.2")
    ax.set_xticks(x); ax.set_xticklabels(ticklabels, fontsize=ANNOT)
...
axes[0].legend(loc="upper center", ncol=3, framealpha=1.0, fontsize=ANNOT, columnspacing=0.8, handlelength=1.4)
...
axes[1].text(1.4, 65.5, "all 64 links", fontsize=ANNOT, color="0.35", ha="right", va="bottom")
...
axes[3].text(3.4, 8.9, "all 8 congested links", fontsize=ANNOT, color="0.35", ha="right", va="bottom")
for a in axes:
