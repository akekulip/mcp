...
for fkey, lab, mk, ls in FLOORS:
    ...
    for m in ms:
        ...
        if med is None:
            ...
            if a < 0.999:
                ax.annotate(f"{a:.2f}", (m, ep), textcoords="offset points", xytext=(5, 3), fontsize=ANNOT, color="0.3")
...
ax.text(1.05, CEIL * 1.12, "budget: 80 epochs", fontsize=ANNOT, color="0.35", ha="left", va="bottom")
