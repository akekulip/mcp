...
parser EgParser(packet_in pkt, out eg_headers_t hdr, out eg_md_t md,
    ...
    state start {
        ...
        md.tx_count = 0;
        md.eg_rnd = 0;
        md.fl_dmac  = 48w0xA5A5A5A5A5A5;
        md.fl_smac  = 48w0x0200000_04D43;
        md.fl_etype = ETYPE_MCP_MIRROR;
        md.fl_bank  = 0;
        md.fl_pad   = 0;
        md.fl_sid   = 0;
...
control Egress(inout eg_headers_t hdr, inout eg_md_t md,
    ...
    /* FLIP EVENT: the one packet per epoch whose count has zero in the low k bits
     * carries a mirror to the collector, so the controller learns the seal instant
     * without polling.  Ternary mask = N-1, value = 0. */
    action flip_mirror(bit<8> b, MirrorId_t sid) {
        md.fl_bank = b;
        md.fl_sid  = sid;
        eg_dprsr_md.mirror_type = 3w1;
    }
    table tbl_clf_flip {
        key     = { md.tx_count : ternary; }
        actions = { flip_mirror; @defaultonly NoAction; }
        size    = 8;
        const default_action = NoAction();
    }
    table tbl_clf_bank {
    ...
    apply {
        if (hdr.csig.isValid()) {
            ...
            tbl_clf_flip.apply();     /* one mirror per epoch boundary */
...
control EgDeparser(packet_out pkt, inout eg_headers_t hdr, in eg_md_t md,
    ...
    Mirror() flip_mir;
    apply {
        if (eg_dprsr_md.mirror_type == 3w1) {
            flip_mir.emit<flip_h>(md.fl_sid,
                { md.fl_dmac, md.fl_smac, md.fl_etype,
                  md.sublink, md.tx_count, md.fl_bank, md.fl_pad });
        }
        pkt.emit(hdr);
    }
}
