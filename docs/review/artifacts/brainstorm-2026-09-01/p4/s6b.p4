...
struct ig_md_t {
    ...
    bit<32> led_hi;
    bit<32> led_gap;     // seq - hi: 0/1 in order, >1 opened a gap, negative = late
    bit<8>  led_flag;    // set by tbl_wit_verdict when this packet opened a gap
}
...
control Ingress(inout headers_t hdr, inout ig_md_t md,
    ...
    Register<wit_led_t, bit<16>>(1024) reg_wit_ledger;
    RegisterAction<wit_led_t, bit<16>, bit<32>>(reg_wit_ledger) wit_ledger = {
        void apply(inout wit_led_t v, out bit<32> rv) {
            rv = v.hi;                 /* pre-update high-water mark; the gap is
                                        * computed in the MAU, because a paired
                                        * register cannot send a subtraction to
                                        * the SALU output. */
            if ((int<32>)(v.hi - hdr.witness.seq) < 0) { v.hi = hdr.witness.seq; }
            v.lo = v.lo + 1;
        }
    };

    action wit_measure() { md.led_hi = wit_ledger.execute(md.wit_link); }
    action wit_gap()     { md.led_gap = hdr.witness.seq - md.led_hi; }
    table tbl_wit_gap {
        key     = { md.role : exact; }
        actions = { wit_gap; @defaultonly NoAction; }
        size    = 4;
        const default_action = NoAction();
        const entries = { ROLE_LOOP : wit_gap(); }
    }

    DirectCounter<bit<64>>(CounterType_t.PACKETS_AND_BYTES) wit_ctr;
    action wit_ok()   { wit_ctr.count(); }
    action wit_late() { wit_ctr.count(); }
    action wit_loss() { wit_ctr.count(); md.led_flag = 1; }
    ...
    table tbl_wit_verdict {
        key      = { md.led_gap : ternary; }
        actions  = { wit_ok; wit_late; wit_loss; }
        counters = wit_ctr;
        size     = 8;
        const default_action = wit_loss();
        const entries = {
            32w0 &&& 32w0xFFFFFFFF          : wit_ok();
            32w1 &&& 32w0xFFFFFFFF          : wit_ok();
            32w0x80000000 &&& 32w0x80000000 : wit_late();
        }
    }
    ...
    apply {
        ...
        if (md.role == ROLE_HOST && hdr.fabric.isValid()) {
            ...
            if (hdr.witness.isValid()) {
                ...
                if (md.led_flag != 0) {          /* a discontinuity arms the fast loop */
                    tbl_wit_arm.apply();
                }
            }
...
control Egress(inout eg_headers_t hdr, inout eg_md_t md,
    ...
    Register<bit<32>, bit<16>>(1024, 0) reg_wit_seq;
    RegisterAction<bit<32>, bit<16>, bit<32>>(reg_wit_seq) wit_next = {
    ...
    Register<bit<8>, bit<16>>(512, 0) reg_tx_frontier;
    ...
    table tbl_eg_fail {
        key = {
            md.sublink           : exact;
            hdr.witness.seq[15:0] : range;   /* Class 2: a 32-bit range key is illegal */
        }
