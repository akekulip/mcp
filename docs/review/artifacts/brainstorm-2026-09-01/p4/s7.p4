...
struct ig_md_t {
    ...
    wit_result_t wit_result; // gap (observed half now lives in md.led_lo)
    bit<32> led_lo;          // ledger arrivals, never reset
}
...
control Ingress(inout headers_t hdr, inout ig_md_t md,
    ...
    RegisterAction<bit<16>, bit<16>, bit<16>>(reg_wit_expect) wit_check = {
        void apply(inout bit<16> v, out bit<16> rv) {
            ...
            if ((int<16>)(v - hdr.witness.seq) <= 0) { v = hdr.witness.seq + 1; }
        }
    };
    ...
    RegisterAction<bit<32>, bit<16>, bit<32>>(reg_wit_observed) wit_count = {
        void apply(inout bit<32> v, out bit<32> rv) {
            rv = v;
            v  = v |+| 1;          /* never reset: this IS the ledger's lo */
        }
    };

    action wit_count_arrival() {
        md.led_lo = wit_count.execute(md.wit_link);
    }
    ...
    apply {
        ...
        if (md.role == ROLE_HOST && hdr.fabric.isValid()) {
            ...
            /* RX frontier removed: the ledger's lo counts arrivals */
...
control Egress(inout eg_headers_t hdr, inout eg_md_t md,
    ...
    apply {
        if (hdr.csig.isValid()) {
            ...
            md.clf_tx_idx = md.sublink;   /* no bank: the ledger has no epochs */
