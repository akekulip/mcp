...
OUT=${OUT:-$G/results_tuning}; SEEDS=${SEEDS:-"6 7 8 9 10"}; BUDGETS=${BUDGETS:-"4 32"}; LOSS_P=${LOSS_P:-1e-3}; BG_LOSS=${BG_LOSS:-0}
...
run(){ local arm=$1 cfg=$2 budget=$3 seed=$4; shift 4; local d=$OUT/$arm/b$budget/cfg$cfg; mkdir -p $d
  ...
  ( ulimit -f 200000; timeout 600 $H/htsim_uec -topo $H/topologies/fat_tree_128_1os.topo -goal $G/../traces/lulesh_8/lulesh_8.bin -nodes 128 -linkspeed 100000 -mtu 4096 -paths 128 -strat ecmp_host -load_balancing_algo oblivious -q 1000000 -seed $seed -end 100000 -o /dev/null -rto_min_us 300 -mcp_rank_stride 16 -mcp_bg_loss $BG_LOSS -mcp_loss $fault:$LOSS_P -mcp_loss_onset_ms 500 -mcp_epoch_us 100000 -mcp_policy "extern:$T/o:$T/a" -mcp_budget $budget -mcp_log $d/seed$seed.csv.tmp > $d/seed$seed.log 2>&1 ) || true
