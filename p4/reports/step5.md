...
- Two RegisterActions, exactly one executes per packet, selected by `tbl_attn` on `md.exceed`
  ...
  - exceedance: `attn = attn |+| k_up` (saturating), `clean = 0` → a_max = 65535 fixed
  - clean: `if (clean >= n_clean−1 && attn > a_min) {clean=0; attn−=1} else if (clean >= n_clean−1) {clean=0} else {clean+=1}`
  - constants are `RegisterParam`s the controller sets: `p_k_up` (1024), `p_a_min` (256),
    `p_n_clean_m1` (4095). Defaults are placeholders until tuning (PREREG §3.2).
...
2. 5 RegisterParams + a 65535 constant → **4 parameter slots** per register; `a_max` became a
   saturating add (`|+|`) and the cap parameter was dropped.
