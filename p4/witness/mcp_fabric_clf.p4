...
struct ig_md_t {
    bit<16> clf_idx;      // (bank << 8) | sublink -- CLF register index
    bit<8>  clf_rx_prev;  // previous receiver-frontier byte, for diagnostics
...
control Ingress(inout headers_t hdr, inout ig_md_t md,
    ...
    /* ===================== CONTEXT LIVENESS FRONTIER (CLF) =====================
     * C-W4 can only report a discontinuity when a LATER packet in the same context
     * survives.  A context that goes completely dark therefore produces no evidence at
     * all, and silence is indistinguishable from "nothing was sent".  CLF closes that by
     * recording, per directed link, WHICH contexts were seen -- so the source's own
     * activity can prove packets should have appeared.
     *
     * Receiver half.  One byte per behavioural sublink per bank.  It is indexed by
     * md.wit_link, the sublink the witness has ALREADY computed, which is what makes this
     * cheap: a per-link 16-bit mask would need a one-hot 1 << ctx, and this compiler
     * cannot shift a runtime value (the same constraint that forces the control plane to
     * supply vlink << 4 to tbl_eg_vlink).  The control plane packs the per-link mask when
     * it reads, so the BATCHED record still crosses the wire, which is where the
     * O(L) rather than O(LK) argument actually lives.
     *
     * Banked: index = (bank << 8) | sublink, bank carried in a spare hdr.fabric.flags bit,
     * so a reader can compare the INACTIVE bank after a guard interval without racing the
     * epoch boundary. */
    ...
    /* Class 11 again: a stateful action with a PHV index cannot be a table default, so a
     * real key with const entries.  ROLE_LOOP is also the right scope for free -- only a
     * loop-port arrival carries an upstream witness. */
    table tbl_rx_frontier {
        key     = { md.role : exact; }
        actions = { rx_frontier_mark; @defaultonly NoAction; }
        size    = 4;
        const default_action = NoAction();
        const entries = { ROLE_LOOP : rx_frontier_mark(); }
    }
    ...
    apply {
        ...
        if (md.role == ROLE_HOST && hdr.fabric.isValid()) {
            ...
            if (hdr.witness.isValid()) {
                ...
                /* CLF receiver mark.  bank parity rides hdr.fabric.flags bit3, which is
                 * free (bits 0..2 are measured/dropped/corrupted).  The OR builds the
                 * index without a shift of a runtime value. */
                md.clf_idx = (bit<16>)md.wit_link;
