# C-W4: behavioural sublinks — compiles, fits, and the claim holds on the model
...
the downstream trusts the label. That is not a weakness to hide: a corrupted label surfaces as a gap
in the *wrong* sublink, so it is detectable rather than silent.
...
| W4 (witness) | 8 / 3 |
| W4 + arming | 8 / 3 |
| **C-W4 (behavioural sublinks)** | **8 / 4** |

One extra egress stage, on a chip with 12 per gress — 4 ingress and 8 egress still free. The
pre-registered stop condition ("stop if C-W4 does not compile within the remaining stages") does
not fire.
...
`p4/ptf/test_cw4_sublinks.py`, 3/3 passing against the generated `mcp_fabric_cw4`:
...
The mechanism works; the *value* is unmeasured. The decisive experiment is not detection accuracy
but capacity: **at the same unsafe-packet bound, how much useful capacity is retained from a
conditionally faulty link** against whole-link quarantine. That needs the conditional-fault
workloads (asymmetric corruption, the >1 KB fault, direction × size, class-selective) run against
binary quarantine, ordinary W4, witness-stop and an oracle envelope. Nothing here licenses a claim
about capacity until that runs.
