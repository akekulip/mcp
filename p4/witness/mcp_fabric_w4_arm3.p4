...
control Ingress(inout headers_t hdr, inout ig_md_t md,
    ...
    /* ARMING THE FAST LOOP FROM A GAP.
     *
     * The natural shape -- `const default_action = wit_loss()` setting md.exceed -- does NOT
     * work on bf-p4c 9.13.x. The compiler folds tbl_wit_verdict (one const entry + a const
     * default) into a GATEWAY: on gap == 0 it supplies the wit_ok payload, and on a miss it
     * SKIPS the table, so the default action never executes. The model's per-packet trace shows
     * the gap computed (md.wit_gap = 0xffff) and then dropped, with tbl_attn running
     * act_attn_clean. Setting md.exceed in wit_measure and clearing it in wit_ok fails for the
     * same reason (p4/ptf/PTF-MODEL.md).
     *
     * So arm from an EXPLICIT control-flow test, whose gateway runs the table exactly when the
     * gap is non-zero. tbl_wit_verdict keeps its counting role. */
