...
control Ingress(inout headers_t hdr, inout ig_md_t md,
    ...
    /* ARM-ON-MEASURE, DISARM-ON-CONTIGUITY.
     *
     * The obvious shape -- `const default_action = wit_loss()` setting md.exceed -- does NOT
     * work, and the model's per-packet trace says why: bf-p4c folds tbl_wit_verdict (one const
     * entry, const default) into a GATEWAY. On a match it supplies the wit_ok payload; on a
     * miss it reports "Table Ingress.tbl_wit_verdict is inhibited by a gateway condition" and
     * the table does not execute at all, so the const default action never runs. The gap was
     * computed correctly (md.wit_gap = 0xffff for one loss) and then silently dropped.
     *
     * So arm on the measurement and disarm on contiguity, which is the same shape the gateway
     * already implements and costs no extra stage.
     *
     * KNOWN INTERACTION, deliberately left visible: tbl_exceed_csig/tbl_exceed_evid run BEFORE
     * this block, so a contiguous witness packet now clears an exceedance they raised. That is
     * acceptable for validating the witness in isolation; a production integration needs a
     * separate md.wit_bad flag OR-ed into md.exceed rather than this shared write. */
    action wit_measure() { md.wit_gap = wit_check.execute(md.wit_link); md.exceed = 1; }
    ...
    action wit_ok()   { md.exceed = 0; wit_ctr.count(); }
