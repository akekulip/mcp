#!/usr/bin/env bash
# Rebuild mcp_fabric_ledger.p4 from the CURRENT source and run p4/ptf/test_ledger.py against
# the laptop's local Tofino software model (SDE 9.13.1).
#
# NOTHING HERE TOUCHES THE SHARED PHYSICAL TOFINO. No ssh, no scp, no chip access: this
# starts tofino-model + bf_switchd on this machine only. The only privileged operations are
# the local model/switchd/PTF launches, which need CAP_NET_RAW on the veths.
#
# Structure follows run_gap_event.sh / run_context_regressions.sh: compile the exact current
# source every run (a fixed prebuilt config silently loads a stale schema), generate a matching
# temporary conf, refuse to run if any model process is already alive, and only ever kill a PID
# this invocation recorded together with its own config path.
set -u
...
BUILD_DIR=$(mktemp -d /tmp/mcp-ledger-model.XXXXXX)
TEST_DIR=$BUILD_DIR/ptf
CONF=$BUILD_DIR/model_ledger.conf
...
mkdir -p "$TEST_DIR"
cp "$REPO_DIR/p4/ptf/test_ledger.py" "$TEST_DIR/test.py"
...
# The single quotes are intentional: the privileged child shell writes its own post-sudo PID.
...
S sh -c 'pidfile=$1; shift; echo $$ > "$pidfile"; exec "$@"' sh "$MODEL_PIDFILE" \
    ...
    > "$MODEL_DIR/ledger.model.log" 2>&1 &
sleep 6
# shellcheck disable=SC2016
S sh -c 'pidfile=$1; shift; echo $$ > "$pidfile"; exec "$@"' sh "$SWITCHD_PIDFILE" \
    ...
    > "$MODEL_DIR/ledger.switchd.log" 2>&1 &

if runner_wait_ready "$SWITCHD_PIDFILE" "$MODEL_PIDFILE" "$MODEL_DIR/ledger.switchd.log" "$MODEL_DIR/ledger.model.log" 7777 "$PROG" 180; then
    echo "=== ready=1 ==="
else
    echo "=== ready=0 ==="
    exit 1
...
printf '%s\n' "$SUDO_PASS" | sudo -S -p '' -E env "SDE=$SDE" "SDE_INSTALL=$SDE_INSTALL" \
    ...
    -t "$TEST_DIR" -f "$MODEL_DIR/ports.json" --arch tofino \
